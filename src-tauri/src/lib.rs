...
#[tauri::command]
pub fn update_customer(state: State<DbState>, id: i64, customer: NewCustomer) -> Result<(), String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
...
#[tauri::command]
pub fn get_customer_debts(state: State<DbState>, customer_id: i64) -> Result<Vec<Debt>, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    let mut stmt = conn
        .prepare(
            "SELECT d.id, d.customer_id, c.name, c.phone, d.sale_id, s.invoice_number,
...
#[tauri::command]
pub fn add_debt_payment(state: State<DbState>, payment: NewDebtPayment) -> Result<(), String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn get_sales_chart(state: State<DbState>, days: i64) -> Result<Vec<SalesChartData>, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn get_top_products(state: State<DbState>, days: i64) -> Result<Vec<TopProduct>, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    let offset = format!("-{} days", days);
